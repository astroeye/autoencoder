...
def build_autoencoder(size=224, latent_dim=(64*2)):
    encoder_input = keras.Input(shape=(size, size, 1), name='img')
    ...
    x = keras.layers.Dense((size*size), activation="relu")(decoder_input)
    decoder_output = keras.layers.Reshape((size, size, 1))(x)
